...
describe RunThisAsync::AsyncRunner do
  ...
  describe '#perform' do
    ...
    before do
      ...
      allow(RunThisAsync::Callee::Decoder).to receive(:call).
        with(callee).and_return(decoded_callee)
    end
    ...
    context 'without expecting a certain job id' do
      ...
      context 'one method' do
        it 'calls a method on a said callee with args' do
          expect(decoded_callee).to receive(method).with(arg, arg2)

          subject.perform(expected_job_id, callee, method, [arg, arg2])
        end

        it 'calls a nethod on a said callee without args' do
          expect(decoded_callee).to receive(method).with(no_args)
    ...
    context 'with a certain job id expected' do
      context 'unexpected job id' do
        ...
        it 'does not call any methods' do
          expect(decoded_callee).not_to receive(method)
      ...
      context 'expected job id' do
        it 'calls methods alright' do
          expect(decoded_callee).to receive(method)
